...
\[
  \begin{aligned}
    \op{cell}(\op{row}, \op{col}) = \begin{cases}
                                      1      & \text{se}~\#\op{possible\_values}(\op{row}, \op{col}) = 2 \\
                                      0      & \text{se}~\#\op{possible\_values}(\op{row}, \op{col}) = 1 \\
                                      \infty & \text{se}~\#\op{possible\_values}(\op{row}, \op{col}) = 0
                                    \end{cases}
     & , &
    h(\op{state}) = \sum_{\substack{0\leq \op{row} < N \\0\leq \op{col} < N}} \op{cell}_{\op{state}}(\op{row}, \op{col})
  \end{aligned}
\]
